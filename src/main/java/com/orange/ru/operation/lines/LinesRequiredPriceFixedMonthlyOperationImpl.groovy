...
/**
 * Здесь расчет разовой желаемой цены, если задана желаемая ежемесячная цена.
 */
@Service("required_price_with_monthly_fixed_operation")
@Repository
@Transactional(readOnly = false)
class LinesRequiredPriceFixedMonthlyOperationImpl implements LinesRequiredPriceFixedMonthlyOperation {
  ...
  void call(ProductItem item) {
    ...
    item.addMoney("inout_required_price_payment_onetime_money",
  ...
  def static Closure fit = { item ->
    ...
    double gm, pb, /* входное значение mrp */ otp = 0; int k = 10000
    ...
    otp.round(-1) //-> inout_required_price_payment_onetime_money
  }
  def static Closure notFit = { item ->
    item.addMoney("inout_required_price_payment_onetime_money",0)
    double mrc = item.get("out_cost_monthly_money").amount,
        r = item.get("ref_risk_ratio"), ref_gm = item.get("ref_standard_required_price_margin_ratio")
