...
INSERT INTO ACCESS_LINES(TYPE_ID,SITE_ID) SELECT 200, 50 from dual;
-- создаем PRODUCT_ITEM: заполняем обертку для услуги TYPE_ID=100(new) и сразу связываем PRODUCT_ITEM и услугу
INSERT INTO PRODUCT_ITEM(TYPE_ID,PRODUCT_ID,NOTE) SELECT 100, max(al.id), 'Access Lines - test product item' FROM ACCESS_LINES al;

-- связываем сценарий и product item.
insert into SCENARIO_PRODUCTS (PRODUCT_ITEM_ID, SCENARIO_ID) SELECT max(pi.id), max(sce.id) FROM PRODUCT_ITEM pi, SCENARIO sce;
...
insert into WRAPPER_MONEY(AMOUNT,CURRENCY,IDENTIFICATION_ID) SELECT 150000,'RUR', ids.id from (SELECT ID as id FROM IDENTIFICATION where CODE='in_capex_equipment_money') ids;
...
insert into WRAPPER_MONEY(AMOUNT,CURRENCY,IDENTIFICATION_ID) SELECT 260000,'RUR', ids.id from (SELECT ID as id FROM IDENTIFICATION where CODE='in_capex_build_money') ids;
insert into WRAPPER_MONEY_PARENT(PARENT_ID, WRAPPER_MONEY_ID) SELECT PRODUCT_ITEM_SEQ.currval, WRAPPER_MONEY_SEQ.currval FROM DUAL;
...
insert into WRAPPER_MONEY_PARENT(PARENT_ID, WRAPPER_MONEY_ID) SELECT PRODUCT_ITEM_SEQ.currval, WRAPPER_MONEY_SEQ.currval FROM DUAL;
insert into WRAPPER_MONEY(AMOUNT,CURRENCY,IDENTIFICATION_ID) SELECT 1110000,'RUR', ids.id from (SELECT ID as id FROM IDENTIFICATION where CODE='in_cost_monthly_build_money') ids;
insert into WRAPPER_MONEY_PARENT(PARENT_ID, WRAPPER_MONEY_ID) SELECT PRODUCT_ITEM_SEQ.currval, WRAPPER_MONEY_SEQ.currval FROM DUAL;
insert into WRAPPER_MONEY(AMOUNT,CURRENCY,IDENTIFICATION_ID) SELECT 230000,'RUR', ids.id from (SELECT ID as id FROM IDENTIFICATION where CODE='in_cost_monthly_lease_money') ids;
insert into WRAPPER_MONEY_PARENT(PARENT_ID, WRAPPER_MONEY_ID) SELECT PRODUCT_ITEM_SEQ.currval, WRAPPER_MONEY_SEQ.currval FROM DUAL;
insert into WRAPPER_MONEY(AMOUNT,CURRENCY,IDENTIFICATION_ID) SELECT 5000,'RUR', ids.id from (SELECT ID as id FROM IDENTIFICATION where CODE='out_standard_price_payment_monthly_money') ids;
insert into WRAPPER_MONEY_PARENT(PARENT_ID, WRAPPER_MONEY_ID) SELECT PRODUCT_ITEM_SEQ.currval, WRAPPER_MONEY_SEQ.currval FROM DUAL;
...
insert into WRAPPER_MONEY_PARENT(PARENT_ID, WRAPPER_MONEY_ID) SELECT PRODUCT_ITEM_SEQ.currval, WRAPPER_MONEY_SEQ.currval FROM DUAL;
insert into WRAPPER_MONEY(AMOUNT,CURRENCY,IDENTIFICATION_ID) SELECT 2000,'RUR', ids.id from (SELECT ID as id FROM IDENTIFICATION where CODE='inout_required_price_payment_monthly_money') ids;
insert into WRAPPER_MONEY_PARENT(PARENT_ID, WRAPPER_MONEY_ID) SELECT PRODUCT_ITEM_SEQ.currval, WRAPPER_MONEY_SEQ.currval FROM DUAL;
