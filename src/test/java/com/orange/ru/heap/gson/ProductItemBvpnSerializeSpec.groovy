...
@Transactional
class ProductItemBvpnSerializeSpec extends spock.lang.Specification {
  Properties props; ProductItem item; String json
  ...
  def setup(){
    ...
    item.scenario.ownerEmail = 'sergey.bogachek@orange.com'
    // item.type
  ...
  def "Превращение готового ProductItem в Json"() {
    when:
    json = GsonUtil.gson.toJson(item)
    println json

    JsonParser parser = new JsonParser();
    JsonElement o1 = parser.parse(json);

    Path resPath = Paths.get(ProductItemBvpnSerializeSpec.class.getResource("../test1.json").toURI());
