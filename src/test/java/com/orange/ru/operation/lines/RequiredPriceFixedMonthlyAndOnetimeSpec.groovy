...
class RequiredPriceFixedMonthlyAndOnetimeSpec extends Specification {
  Properties props; ProductItem item
  @Autowired LinesRequiredPriceFixedMonthlyAndOnetimeOperation required_price_operation
  ...
  def "Маржа, когда заданы желаемые ежемесячная и разовые цены"() {
    expect: equals(item.get("out_required_price_margin_percent"), parse(props.get("out_required_price_margin_percent")), 1)
  }
  def "Срок окупаемости, когда заданы желаемые ежемесячная и разовые цены"() {
    expect: equals(item.get("out_required_price_payback_months"), parse(props.get("out_required_price_payback_months")), 1)
  }
