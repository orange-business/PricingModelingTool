...
class RequiredPriceFixedMonthlySpec extends Specification {
  Properties props; ProductItem item
  @Autowired LinesRequiredPriceFixedMonthlyOperation required_price_operation
  ...
  def "Желаемая цена, разовый платеж (ежемесячный задан)"() {
    expect: equals(item.get("inout_required_price_payment_onetime_money").amount, parse(props.get("inout_required_price_payment_onetime_money")), 1)
  }
